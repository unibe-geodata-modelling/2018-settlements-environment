...
arcpy.env.cellSize = dem #Cellsize setting (environment setting)
arcpy.env.extent = dem #Setting of the extent (envorinment setting)
...
#Focal Stastics DEM: calculating the standard deviation for each cell from the 10x10 surrounding cells
...
#halftpi10x10std= (tpi10x10std/2) +++++ This steps creates an exit code in Python, however in the arcpy console in ArcGIS and with other IDE this command works
#halftpi10x10std.save(myworkspace + "/" + "Results" + "/" + "hstd.tif") +++++ This steps creates an exit code in Python, however in the arcpy console in ArcGIS and with other IDE this command works
halftpi10x10std =arcpy.Raster(myworkspace + "/" + "Results" + "/" + "hstd.tif") #Reading in the newly created Raster

#Classification with Map Algebra using the conditional evaluation -> This steps creates an exit code in Python, however in the arcpy console in ArcGIS and with other IDE this command works
...
#classification.save(myworkspace + "/" + "Results" + "/" + "classif.tif") ++++++ This steps creates an exit code in Python, however in the arcpy console in ArcGIS and with other IDE this command works
...
arcpy.Resample_management(classification, myworkspace + "/" + "Results" + "/" + "classif200.tif", "200 200", "NEAREST") #Resampling raster to a coarser resolution (200x200m cell size), because there is an error message if the resulting shapefile exeeds the size of 2 GB
classification200=arcpy.Raster(myworkspace + "/" + "Results" + "/" + "classif200.tif") #Reading in the resampled raster


#Convert classified Raster to Polygon for later use for the intersect function
classif200= myworkspace + "/" + "Results" + "/" + "classif200.shp" #Preparing variable for the shapefile output
...
#Erase function: This function is used because the lakes were also classified as a landform. Because only the information of the land is of interest the erase function is used to erase all the lakes from the shapefile with the classified landforms.
...
#Setting the buffer around the settlements with various radii
...
#Intersect buffer and the landform shapefile
...
arcpy.CalculateField_management(interland500, "Percent", "[POLY_AREA] /785191.17*100", "VB", "")#The Area of the different landform classes inside the buffer is divided by the whole buffer area and multiplied by 100 to receive the percentage of coverage per polygon
...
#Variables for calculating the mean surface coverage in percents per landform per buffer
...
#Calculate the mean percentage (and the standard deviation) of land coverage of the  individual landforms.
