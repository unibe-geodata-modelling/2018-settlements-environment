...
*****************************************************************************************************************************
Variable "myworkspace", "dem", "Settlements", "Lakes", "Buffer500" to "Buffer5000", "interland500" to "interland 500",
...
The lake shapfile is read in in line 90. You have to adjust the path to where you have stored your lake shapefile.

The "classif200land" variable (line 93) is used for the output of the erase function. You have to adjust
...
The variables "interland500" to "interland5000" (lines 127 to 129) are the variables that are used for the ouptu path of the intersect function.
...
On line 32 the mean is determined and on line 36 the standard deviation. For both you just have to adjust the path (second input in the brackets), to your chosen
...
For the slope function you only have to adjust the path where you want to save it (second input into the bracket on line 45) and use the same path for line
46 to read in the newly created slope raster. The slope is given in Degrees, you should not change that since this is important for the later classification
...
The TPI is simply the difference between the DEM and the mean elevation raster that was determined in the lines 32-37.
...
In the classification half the value from the standard deviation raster from the lines 36 to 37 is used several times. To make the writing of the classification
a bit easier a separate raster is created for this instead of dividing the standard deviation in the classification itself.
...
Weiss (2001) suggested in his poster presentation six classifications which are displayed on line 64. Line 64 is the conditional evaluation used for the classification,
and line 65 would save the newly calssifeid raster. However, as with the half standard deviation raster this creates an exit code in Pycharm. It works however as well in
the python console in ArcGIS or with other IDEs. In any case, n line 65 you have to adjust the path to your chosen directory.
...
an error. There would be also another work around if you would save the polygon in a geodatabase. You can also skip this step/ put a # before this command lines if you
already have a coarser resolution that does not lead to a shapefile of that size. If not you have to adjust the path on line 76 (second input in the bracket of the function)
to your chosen directory and use the same path in line 77 to read in the newly created coarser raster. Furthermore you can adjust the cell size, which is the second last input in 
the function bracket. In this case 200 200 was chosen -> a resolution of 200x200m.

If you skip this step you have to be aware that you have to put in the variable "classification" and not "classification200" on the line 82 as the first input into the brackets.
...
In the last input in the bracket of the function you have to change the last field name. In this case the field name for the settlements was "Fundort". You have to replace this with your correspondending field name. 
